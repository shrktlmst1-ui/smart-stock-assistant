"""فرصة الآن — scores scanner cache snapshots for sub-$10 momentum plays."""
...
def _collect_snapshots() -> list[StockSnapshot]:
    ...
    for sym in dict.fromkeys(symbols):
        ...
        if not snap or sym in seen:
            continue
        seen.add(sym)
        if MIN_PRICE_USD < snap.price <= MAX_PRICE_USD:
            result.append(snap)
    return result
...
def _snapshot_to_signal(snap: StockSnapshot, *, market_open: bool) -> OpportunityNowSignal | None:
    ...
    status = _status_from_score(score, safety_failed=safety_failed, market_open=market_open)
    if not market_open and status == "فرصة الآن":
        status = "مراقبة"
...
def get_opportunity_now() -> OpportunityNowResponse:
    """Build فرصة الآن list from scanner cache only."""
    try:
        ...
        for snap in _collect_snapshots():
            try:
                sig = _snapshot_to_signal(snap, market_open=market_open)
                if sig:
                    signals.append(sig)
            except Exception as exc:
                logger.warning("Opportunity now skip %s: %s", snap.symbol, type(exc).__name__)

        signals.sort(key=lambda s: s.score, reverse=True)
        top = next((s for s in signals if s.status == "فرصة الآن"), None)
        if top is None:
        ...
        return OpportunityNowResponse(
            ...
            message=message,
            signals=signals,
            top_signal=top if top and top.score > 0 and top.price > 0 else None,
        )
    except Exception as exc:
        ...
        return OpportunityNowResponse(
            ...
            market_open=False,
            message="تعذر تحميل فرصة الآن مؤقتاً",
            signals=[],
            top_signal=None,
        )
